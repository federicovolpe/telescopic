//V 2.0 : il pid gestisce la pwm del motore (range operativita' 130, 255)
...
volatile double pwm = 0;                                      // valore della pwm
volatile double offset = 0;                                   // la distanza dall'obiettivo

// setup per il pid
#include <PID_v1.h>
float P = 1, I = .2, D = .2;
PID PIDcontroller (&distanza_norm, &pwm, &obiettivo ,P ,I ,D, REVERSE);
...
void setup() {
  ...
  // impostati a low poiche' il mio arduino a volte li fa partire HIGH
...
void movimento() { // calcola il tempo di movimento del motore
  ...
  // viene utilizzata la distanza normalizzata e non l'offset dall'obiettivo perche' il pid
  // gia' di suo calcola l'offset
  ...
  pwm = map(pwm, 1, 50, 130, 200); // i parametri vengono cambiati dato il range di operativita'
  analogWrite(pwmPin, pwm); // scrittura del parametro pwm 
}
...
void stampaParametri () {
  ...
  if ( direzione == 1 ){   Serial.println(pwm);  }
  else{                   Serial.println(- pwm);  }
